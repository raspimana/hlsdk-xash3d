...
class NintendoSwitch:
	...
	def cflags(self, cxx = False):
		cflags = []
		# arch flags
		cflags += ['-D__SWITCH__', '-march=armv8-a+crc+crypto', '-mtune=cortex-a57', '-mtp=soft', '-ftls-model=local-exec', '-fPIE', '-fPIC']
		# help the linker out
	...
	# they go before object list
	def linkflags(self):
		linkflags = self.cflags()
		linkflags += ['-specs=%s/switch.specs' % self.libnx_dir]
